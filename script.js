...
function addToDisplay(value) {
    currentExpression = currentExpression.slice(0, cursorPosition) + value + currentExpression.slice(cursorPosition);
...
function getSubExpressions(expression) {
    ...
    // 1. ดึงตัวแปรเดี่ยวออกมาก่อน (p, q, r, s) เรียงตามตัวอักษร
    const variables = (expression.match(/[pqrs]/g) || []);
    [...new Set(variables)].sort().forEach(v => {
        result.push(v);
    });
    
    // 2. ดึงนิเสธของตัวแปรเดี่ยว (~p, ~q, ~r, ~s)
    ...
    // 3. ดึงนิพจน์ในวงเล็บ (เฉพาะเนื้อหาข้างใน)
    ...
    // 4. ดึงนิเสธของนิพจน์ในวงเล็บ (~(q∨s))
    ...
    // 5. สำหรับประพจน์ที่ไม่มีวงเล็บ - เรียงตามลำดับความสำคัญ (precedence)
            ...
            if (expression.includes(op)) {
                ...
                for (let i = 0; i < parts.length - 1; i++) {
                    ...
                    if (subExpr && !result.includes(subExpr) && subExpr !== expression) {
                        // ตรวจสอบว่าเป็นนิพจน์ย่อยที่สมบูรณ์
